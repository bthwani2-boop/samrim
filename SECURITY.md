...
This file owns vulnerability-reporting and repository secret-handling guidance only. Durable application/security engineering requirements are owned by the exact pinned `governance/policy/SECURITY.md` in `bthwani2-boop/governance-and-docs`, resolved through `knowledge.sources.json`; current implementation/security state is proven from executable source, configuration, runtime and evidence.
