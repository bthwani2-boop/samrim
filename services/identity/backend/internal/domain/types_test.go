...
func TestCanonicalRoleSurfaceMapping(t *testing.T) {
	...
	for _, retired := range []string{"platform_owner", "operator_owner", "employee"} {
...
func TestTrustedCallerProvisionBoundary(t *testing.T) {
	...
	denied := [][2]string{{"dsh", "operator"}, {"control-panel", "client"}, {"control-panel", "captain"}, {"control-panel", "platform_owner"}, {"browser", "operator"}}
...
func TestTrustedCallerOperationBoundaries(t *testing.T) {
	...
	for _, pair := range [][2]string{{"control-panel", "platform_owner"}, {"dsh", "platform_owner"}, {"browser", "operator"}} {
...
func TestManagedRoleBoundary(t *testing.T) {
	...
	for _, role := range []string{"client", "operator", "platform_owner", "employee", ""} {
		if IsManagedRole(role) {
...
func TestManagedActivationRoleBoundary(t *testing.T) {
	...
	for _, role := range []string{"client", "platform_owner", "operator_owner", "employee", ""} {
		if IsManagedActivationRole(role) {
			t.Fatalf("unexpected managed activation role %q", role)
		}
	}
...
func TestControlPanelRoleBoundary(t *testing.T) {
	...
	for _, role := range []string{"platform_owner", "operator_owner", "client", "partner", "captain", "field", ""} {
