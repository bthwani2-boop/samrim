...
func (s *Service) deliverNext(ctx context.Context) (bool, error) {
	...
	if _, err := tx.ExecContext(ctx, "UPDATE identity_challenge_deliveries d SET status='expired',finished_at=clock_timestamp(),updated_at=clock_timestamp() FROM identity_challenges c WHERE d.challenge_id=c.id AND d.status='pending' AND c.expires_at<=clock_timestamp()"); err != nil {
		return false, err
	}
	if _, err := tx.ExecContext(ctx, "UPDATE identity_challenge_deliveries d SET status='suppressed',finished_at=clock_timestamp(),updated_at=clock_timestamp() FROM identity_challenges c WHERE d.challenge_id=c.id AND d.status='pending' AND c.status<>'pending'"); err != nil {
		return false, err
	}
	...
	code, err := s.codeFor(item.challengeID, item.purpose)
	if err != nil {
