...
var identitySchemaConstraints = []schemaConstraint{
	...
	{table: "identity_actor_roles", name: "identity_actor_role_activation_check", definition: "CHECK ((((role)::text = ANY ((ARRAY['partner'::character varying, 'captain'::character varying, 'field'::character varying, 'operator'::character varying])::text[])) OR (activated_at IS NULL)))", critical: true},
	{table: "identity_actor_roles", name: "identity_actor_role_check", definition: "CHECK (((role)::text = ANY ((ARRAY['client'::character varying, 'partner'::character varying, 'captain'::character varying, 'field'::character varying, 'operator'::character varying])::text[])))", critical: true},
	...
	{table: "identity_password_credentials", name: "identity_password_credential_role_check", definition: "CHECK (((role)::text = ANY ((ARRAY['client'::character varying, 'partner'::character varying, 'captain'::character varying, 'field'::character varying])::text[])))", critical: true},
	...
	{table: "identity_challenges", name: "identity_challenge_purpose_check", definition: "CHECK (((purpose)::text = ANY ((ARRAY['client_register'::character varying, 'client_recover'::character varying, 'managed_activate'::character varying, 'operator_enroll'::character varying, 'operator_recover'::character varying])::text[])))", critical: true},
	{table: "identity_challenges", name: "identity_challenge_purpose_role_check", definition: "CHECK (((((purpose)::text = ANY ((ARRAY['client_register'::character varying, 'client_recover'::character varying])::text[])) AND ((role)::text = 'client'::text)) OR (((purpose)::text = 'managed_activate'::text) AND ((role)::text = ANY ((ARRAY['partner'::character varying, 'captain'::character varying, 'field'::character varying])::text[]))) OR (((purpose)::text = ANY ((ARRAY['operator_enroll'::character varying, 'operator_recover'::character varying])::text[])) AND ((role)::text = 'operator'::text))))", critical: true},
	{table: "identity_challenges", name: "identity_challenge_role_check", definition: "CHECK (((role)::text = ANY ((ARRAY['client'::character varying, 'partner'::character varying, 'captain'::character varying, 'field'::character varying, 'operator'::character varying])::text[])))", critical: true},
	...
	{table: "identity_sessions", name: "identity_session_role_check", definition: "CHECK (((role)::text = ANY ((ARRAY['client'::character varying, 'partner'::character varying, 'captain'::character varying, 'field'::character varying, 'operator'::character varying])::text[])))", critical: true},
	...
	{table: "identity_password_attempts", name: "identity_password_attempt_role_check", definition: "CHECK (((role)::text = ANY ((ARRAY['client'::character varying, 'partner'::character varying, 'captain'::character varying, 'field'::character varying])::text[])))", critical: true},
	...
	{table: "identity_bootstrap_state", name: "identity_bootstrap_state_initial_operator_actor_id_fkey", definition: "FOREIGN KEY (initial_operator_actor_id) REFERENCES identity_actors(id) ON DELETE RESTRICT", critical: true},
	{table: "identity_webauthn_users", name: "identity_webauthn_users_pkey", definition: "PRIMARY KEY (rp_id, actor_id)", critical: true},
	{table: "identity_webauthn_users", name: "identity_webauthn_user_actor_fk", definition: "FOREIGN KEY (actor_id) REFERENCES identity_actors(id) ON DELETE CASCADE", critical: true},
	{table: "identity_webauthn_users", name: "identity_webauthn_user_handle_uq", definition: "UNIQUE (rp_id, user_handle)", critical: true},
	{table: "identity_webauthn_credentials", name: "identity_webauthn_credentials_pkey", definition: "PRIMARY KEY (rp_id, credential_id)", critical: true},
	{table: "identity_webauthn_credentials", name: "identity_webauthn_credential_actor_fk", definition: "FOREIGN KEY (actor_id) REFERENCES identity_actors(id) ON DELETE CASCADE", critical: true},
	{table: "identity_webauthn_credentials", name: "identity_webauthn_credential_sign_count_check", definition: "CHECK ((sign_count >= 0))", critical: true},
	{table: "identity_webauthn_ceremonies", name: "identity_webauthn_ceremonies_pkey", definition: "PRIMARY KEY (id)", critical: true},
	{table: "identity_webauthn_ceremonies", name: "identity_webauthn_ceremony_kind_check", definition: "CHECK ((kind = ANY (ARRAY['operator_registration'::text, 'operator_recovery_registration'::text, 'operator_authentication'::text])))", critical: true},
	...
	{table: "identity_operator_permissions", name: "identity_operator_permissions_permission_chk", definition: "CHECK (((permission)::text = ANY ((ARRAY['operations'::character varying, 'partners'::character varying, 'catalog'::character varying, 'marketing'::character varying, 'finance'::character varying, 'platform_policies'::character varying])::text[])))", critical: true},
	...
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_versions_pkey", definition: "PRIMARY KEY (actor_id, version)", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_versions_actor_id_fkey", definition: "FOREIGN KEY (actor_id) REFERENCES identity_actors(id) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_versions_submitted_by_actor_id_fkey", definition: "FOREIGN KEY (submitted_by_actor_id) REFERENCES identity_actors(id) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_versions_verified_by_actor_id_fkey", definition: "FOREIGN KEY (verified_by_actor_id) REFERENCES identity_actors(id) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_version_chk", definition: "CHECK ((version > 0))", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_parts_chk", definition: "CHECK ((((length(btrim(given_name)) >= 1) AND (length(btrim(given_name)) <= 80)) AND ((length(btrim(second_name)) >= 1) AND (length(btrim(second_name)) <= 80)) AND ((length(btrim(third_name)) >= 1) AND (length(btrim(third_name)) <= 80)) AND ((length(btrim(family_name)) >= 1) AND (length(btrim(family_name)) <= 80))))", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_status_chk", definition: "CHECK ((status = ANY (ARRAY['PENDING_VERIFICATION'::text, 'VERIFIED'::text, 'REJECTED'::text])))", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_source_chk", definition: "CHECK ((source = 'GOVERNED_IDENTITY_DOCUMENT_REVIEW'::text))", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_evidence_chk", definition: "CHECK (((length(btrim(evidence_reference)) >= 1) AND (length(btrim(evidence_reference)) <= 512)))", critical: true},
	{table: "identity_actor_legal_name_versions", name: "identity_actor_legal_name_verified_chk", definition: "CHECK ((((status = 'VERIFIED'::text) AND (verified_by_actor_id IS NOT NULL) AND (verified_at IS NOT NULL) AND ((length(btrim(verification_evidence_reference)) >= 1) AND (length(btrim(verification_evidence_reference)) <= 512)) AND (verified_by_actor_id <> submitted_by_actor_id)) OR ((status <> 'VERIFIED'::text) AND (verified_by_actor_id IS NULL) AND (verified_at IS NULL) AND (verification_evidence_reference IS NULL))))", critical: true},
	{table: "identity_actor_legal_names", name: "identity_actor_legal_names_pkey", definition: "PRIMARY KEY (actor_id)", critical: true},
	{table: "identity_actor_legal_names", name: "identity_actor_legal_names_actor_id_fkey", definition: "FOREIGN KEY (actor_id) REFERENCES identity_actors(id) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_names", name: "identity_actor_legal_names_current_fk", definition: "FOREIGN KEY (actor_id, current_version) REFERENCES identity_actor_legal_name_versions(actor_id, version) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_pkey", definition: "PRIMARY KEY (id)", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_acting_actor_id_fkey", definition: "FOREIGN KEY (acting_actor_id) REFERENCES identity_actors(id) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_idempotency_key_key", definition: "UNIQUE (idempotency_key)", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_version_fk", definition: "FOREIGN KEY (actor_id, version) REFERENCES identity_actor_legal_name_versions(actor_id, version) ON DELETE RESTRICT", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_type_chk", definition: "CHECK ((event_type = ANY (ARRAY['LEGAL_NAME_SUBMITTED'::text, 'LEGAL_NAME_VERIFIED'::text])))", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_evidence_chk", definition: "CHECK (((length(btrim(evidence_reference)) >= 1) AND (length(btrim(evidence_reference)) <= 512)))", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_hash_chk", definition: "CHECK ((length(btrim(request_hash)) > 0))", critical: true},
	{table: "identity_actor_legal_name_events", name: "identity_actor_legal_name_events_correlation_chk", definition: "CHECK (((length(btrim(correlation_id)) >= 8) AND (length(btrim(correlation_id)) <= 128)))", critical: true},
}
