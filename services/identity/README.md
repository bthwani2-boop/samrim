...
For `BTHWANI_ENV=development` only, the HTTP composition registers a development-session convenience route. It can mint a normal role-scoped session only when exactly one already-existing enabled, security-enabled, authentication-ready actor/role has completed its canonical activation or enrollment; if more than one candidate is ready it returns `409 CONFLICT` instead of choosing one. It never creates actors, roles or credentials. Mobile and Control use it only when reusable local session state is missing or terminally invalid. Explicit logout or recovery remains signed out for that runtime instance, while a fresh runtime may resume development continuity. This keeps authentication ceremonies out of ordinary local product iteration without allowing the development shortcut to satisfy activation, enrollment, login, or recovery proof.
