...
func LoadMigrations(directory string) ([]MigrationRecord, []string, error) {
	...
	names := []string{"001_payment_intents.sql", "002_cash_remittances.sql"}
...
func VerifySchema(ctx context.Context, db *sql.DB, records []MigrationRecord) error {
	...
	for _, relation := range []string{"wlt.payment_intents", "wlt.payment_intent_events", "wlt.cash_remittances", "wlt.cash_remittance_events"} {
		var exists bool
