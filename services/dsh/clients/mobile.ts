...
export function createDshMobileClient(rawBaseUrl: string, options: DshMobileClientOptions = {}) {
  ...
  return {
    ...
    async createOwnPayoutIntent(accessToken: string, amountMode: "FULL_AVAILABLE" | "SPECIFIED", amountMinor?: number): Promise<PayoutIntentResponse> {
      ...
      return userRequest<PayoutIntentResponse>(accessToken, dshOperationPaths.createOwnPayoutIntent.path, dshOperationPaths.createOwnPayoutIntent.method, { amountMode, ...(amountMinor === undefined ? {} : { amountMinor }) }, mutationHeaders());
    },
    async correctAndResubmitJoiningCase(accessToken: string, caseID: string, input: CorrectJoiningCaseRequest, expectedVersion: number): Promise<JoiningCaseResponse> {
      ...
      const fulfillmentModes = input.firstStoreFulfillmentModes;
      if (!normalized || businessName.length < 2 || businessName.length > 160 || firstStoreName.length < 2 || firstStoreName.length > 160 || !serviceCityId || !firstStoreVerticalId || !Array.isArray(fulfillmentModes) || fulfillmentModes.length === 0 || new Set(fulfillmentModes).size !== fulfillmentModes.length || !Number.isFinite(input.firstStoreLatitude) || !Number.isFinite(input.firstStoreLongitude) || input.firstStoreLatitude < -90 || input.firstStoreLatitude > 90 || input.firstStoreLongitude < -180 || input.firstStoreLongitude > 180 || expectedVersion < 1) throw new Error("DSH_JOINING_CASE_INPUT_INVALID");
      const path = dshOperationPaths.correctAndResubmitJoiningCase.path.replace("{caseId}", encodeURIComponent(normalized));
      return userRequest<JoiningCaseResponse>(accessToken, path, dshOperationPaths.correctAndResubmitJoiningCase.method, { businessName, firstStoreName, serviceCityId, firstStoreVerticalId, firstStoreLatitude: input.firstStoreLatitude, firstStoreLongitude: input.firstStoreLongitude, firstStoreFulfillmentModes: fulfillmentModes }, { ...mutationHeaders(), "X-Expected-Version": String(expectedVersion) });
    },
    ...
    async quoteCheckout(accessToken: string, input: CheckoutRequest, expectedCartVersion: number): Promise<CheckoutQuoteResponse> {
      ...
      if (!input.cartId.trim() || !input.storeId.trim() || (input.fulfillmentMode === "BTHWANI_CAPTAIN" && !addressId) || !input.fulfillmentMode || expectedCartVersion < 1) throw new Error("DSH_CHECKOUT_INPUT_INVALID");
    ...
    async checkoutCart(accessToken: string, input: CheckoutRequest, expectedCartVersion: number): Promise<OrderResponse> {
      const addressId = input.addressId?.trim() ?? "";
      if (!input.cartId.trim() || !input.storeId.trim() || (input.fulfillmentMode === "BTHWANI_CAPTAIN" && !addressId) || !input.fulfillmentMode || expectedCartVersion < 1) throw new Error("DSH_CHECKOUT_INPUT_INVALID");
      return userRequest<OrderResponse>(accessToken, dshOperationPaths.checkoutCart.path, dshOperationPaths.checkoutCart.method, { ...input, cartId: input.cartId.trim(), storeId: input.storeId.trim(), addressId }, { ...mutationHeaders(), "X-Expected-Version": String(expectedCartVersion) });
    },
    async createMultiStoreCheckout(accessToken: string, input: MultiStoreCheckoutRequest): Promise<MultiStoreCheckoutResponse> {
      if (!input.id.trim() || input.children.length < 2 || input.children.length > 10 || input.children.some((child) => !child.cartId.trim() || !child.storeId.trim() || (child.fulfillmentMode === "BTHWANI_CAPTAIN" && !child.addressId.trim()) || child.cartVersion < 1 || !child.fulfillmentMode)) throw new Error("DSH_MULTI_STORE_CHECKOUT_INPUT_INVALID");
