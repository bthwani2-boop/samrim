...
func main() {
	...
	databaseURL := strings.TrimSpace(os.Getenv("DSH_MIGRATION_DATABASE_URL"))
	if databaseURL == "" {
		if environment == "staging" || environment == "production" {
			log.Fatal("DSH_MIGRATION_DATABASE_URL is required outside local environments")
		}
		databaseURL = strings.TrimSpace(os.Getenv("DSH_DATABASE_URL"))
	}
	if _, err := opsafety.RequireOrdinaryCLIEnvironment(environment, "dsh migration"); err != nil {
		log.Fatal(err)
	}
