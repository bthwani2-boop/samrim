...
func ParseDatabaseTarget(raw string) (DatabaseTarget, error) {
	...
	return DatabaseTarget{Host: strings.ToLower(strings.TrimSpace(parsed.Hostname())), Port: port, Name: name, User: strings.TrimSpace(parsed.User.Username())}, nil
}
...
func RequireExpectedDatabaseTarget(raw string, getenv func(string) string) (DatabaseTarget, error) {
	...
	if expected.Host == "" || expected.Port == "" || expected.Name == "" || expected.User == "" {
		return DatabaseTarget{}, fmt.Errorf("explicit database target identity is required via %s, %s, %s and %s", expectedDatabaseHostEnv, expectedDatabasePortEnv, expectedDatabaseNameEnv, expectedDatabaseUserEnv)
	}
	if target != expected {
		return DatabaseTarget{}, fmt.Errorf("database target identity mismatch: observed host=%s port=%s database=%s user=%s; expected host=%s port=%s database=%s user=%s", target.Host, target.Port, target.Name, target.User, expected.Host, expected.Port, expected.Name, expected.User)
	}
	return target, nil
}
