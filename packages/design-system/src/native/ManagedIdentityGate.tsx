...
export type ManagedIdentityGateProps = {
  ...
  activate: (phone: string, activationCode: string, verificationCode: string) => Promise<ManagedSessionState>;
  restore: () => Promise<ManagedSessionState>;
  currentState: () => ManagedSessionState;
  logout: () => Promise<void>;
};

function messageOf(value: unknown, roleLabel: string): string {
  ...
  if (raw.includes("authentication") || raw.includes("unauthorized") || raw.includes("invalid")) return "رمز التفعيل أو رمز تحقق الهاتف غير صحيح. راجع الرمز وحاول مرة أخرى.";
...
export function ManagedIdentityGate({ roleLabel, requestCode, activate, restore, currentState, logout }: ManagedIdentityGateProps) {
  ...
  async function activateDevice() {
    setBusy(true); setError(""); setNotice("");
    try { setState(await activate(phone, activationCode, verificationCode)); setVerificationCode(""); }
    catch (cause) { setError(messageOf(cause, roleLabel)); }
    finally { setBusy(false); }
  }

  async function logoutDevice() { setBusy(true); setError(""); try { await logout(); } catch (cause) { setError(messageOf(cause, roleLabel)); } finally { setState(currentState()); setBusy(false); } }
