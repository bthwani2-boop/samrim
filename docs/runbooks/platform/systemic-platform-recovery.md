...
2. **Current environment recovery procedures**:
   - **Local/integration environments**: Use the repository-owned integration lifecycle only. `pnpm runtime:integration:close` performs target-identity preflight, resets the prior integration stack, rebuilds it, verifies health/readiness and exact Identity schema, and closes the proof run cleanly. When an interactive stack must remain running, use `pnpm runtime:integration:up` followed by `pnpm runtime:integration:verify`; stop it with `pnpm runtime:down`. Do not reference or invent an untracked runtime-open helper.
