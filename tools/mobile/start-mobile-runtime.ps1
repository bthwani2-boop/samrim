...
if (-not [string]::IsNullOrWhiteSpace($adbSerial)) {
    ...
    if ($metroPort) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "ADB reverse failed for $adbSerial port $metroPort." }
    }
    & adb -s $adbSerial reverse "tcp:18082" "tcp:18082" *> $null
    if ($LASTEXITCODE -ne 0) { throw "ADB reverse failed for $adbSerial port 18082." }
    & adb -s $adbSerial reverse "tcp:58080" "tcp:58080" *> $null
    if ($LASTEXITCODE -ne 0) { throw "ADB reverse failed for $adbSerial port 58080." }
