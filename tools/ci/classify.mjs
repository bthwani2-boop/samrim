...
let changed = [];
try {
  if (validBase) changed = execFileSync("git", ["diff","--name-only",`${base}...${head}`], {encoding:"utf8"}).split(/?
/).filter(Boolean);
  else { changed = execFileSync("git", ["show","--pretty=","--name-only",head], {encoding:"utf8"}).split(/?
/).filter(Boolean); base = ""; }
} catch {
  changed = execFileSync("git", ["ls-tree","-r","--name-only",head], {encoding:"utf8"}).split(/?
/).filter(Boolean);
  base = "";
}
