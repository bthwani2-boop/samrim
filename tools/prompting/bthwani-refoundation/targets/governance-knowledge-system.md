...
SYSTEM_CONTEXT=PASS
REPOSITORY_TOPOLOGY_MODEL=PASS
...
Governance closure must classify every material responsibility discovered from target evidence and, when applicable, donor forensic evidence. File count and a large capability catalog are not proof of completeness.

```text
EVERY_MATERIAL_RESPONSIBILITY
→ DURABLE_CAPABILITY | SUBCAPABILITY | POLICY | TECHNICAL_MECHANISM | DERIVED_PROJECTION | EXPLICIT_NON_GOAL
→ ONE_DURABLE_OWNER WHEN DURABLE
→ APPLICABLE_ACTOR/JOURNEY/FAILURE/SECURITY/READBACK MODEL
```

For donor-based clean reconstruction additionally require:

```text
DONOR_DURABLE_MEANING_DISPOSITION=PASS
DONOR_GOVERNANCE_SEMANTIC_ATOMS_ACCOUNTED=PASS
HISTORICALLY_DELETED_REQUIRED_MEANING_LOST=0
DONOR_IMPLEMENTATION_INVENTORY_COPIED_AS_GOVERNANCE=0
```
