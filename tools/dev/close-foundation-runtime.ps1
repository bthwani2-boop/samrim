...
function Stop-ExistingFoundationStack {
    Write-Host ""
    Write-Host "=== Reset previous foundation stack ==="
    $code = Invoke-Compose -Arguments @("down", "--remove-orphans")
    if ($code -ne 0) {
        Fail "Unable to stop previous Foundation compose stack."
    }
}
...
try {
    ...
    Stop-ExistingFoundationStack

    Write-Host ""
    Write-Host "=== Foundation host port preflight ==="
    Verify-HostPortsAvailable -Env $envMap

    Write-Host ""
    Write-Host "=== Foundation compose config ==="
    $null = & docker compose --env-file $envPath -f $composeFile --profile foundation config
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host ""
    Write-Host "=== Verify foundation infrastructure ==="
