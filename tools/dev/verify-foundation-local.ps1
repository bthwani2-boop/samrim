...
try {
    ...
    Assert-CleanTree "local Foundation proof start"
    ...
    Run-NativeStep "Foundation compose config" {
    ...
    Assert-CleanTree "pre-runtime Foundation proof"
    ...
    Assert-CleanTree "local Foundation proof completion"

    Write-Host ""
    Write-Host "FOUNDATION_LOCAL_WINDOWS_PROOF=PASS"
    Write-Host "FOUNDATION_LOCAL_EXPO_CONFIG=PASS"
    Write-Host "FOUNDATION_LOCAL_WORKSPACE=PASS"
    if (-not $SkipRuntime) {
        Write-Host "FOUNDATION_LOCAL_RUNTIME=PASS"
    }
}
finally {
    Pop-Location
