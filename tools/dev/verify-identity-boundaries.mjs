...
const challengeService = has("services/identity/backend/internal/challenge/service.go", ["RequestClientRegistration", "LoginClient", "RequestClientRecovery", "RecoverClient", "RequestManagedActivation", "ActivateManaged", "RequestOperatorEnrollment", "RequestOperatorRecovery", "IssueOperatorEnrollmentToken", "identity_challenge_deliveries", "waitPasswordBackoff"]);
...
const security = has("services/identity/backend/internal/security/values.go", ["argon2.IDKey", "NormalizePassword", "PasswordAllowed", "NormalizeRecoveryCredential", "RandomRecoveryCredential", "NeedsPasswordRehash"]);
if (security.includes("bcrypt")) failures.push("legacy bcrypt remains in Identity security implementation");
has("services/identity/backend/internal/security/password_blocklist.go", ["passwordBlocklistVersion", "commonPasswordBlocklistV1", "PasswordBlocklistVersion"]);
