...
try {
  ...
  if (pkg.exports?.["./presentation"]?.types !== "./clients/presentation/ManagedIdentityFlow.tsx" ||
      pkg.exports?.["./presentation"]?.default !== "./clients/presentation/ManagedIdentityFlow.tsx") {
    failures.push("Identity package presentation export drifted");
  }
} catch {
...
const generatedOperations = read("services/identity/clients/generated/identity-operations.ts");
const generatedGoModels = read("services/identity/clients/go/identity_types_generated.go");
const generatedGoOperations = read("services/identity/clients/go/identity_operations_generated.go");
for (const [body, required] of [
  [generatedOperations, "AUTO-GENERATED from services/identity/contracts/identity.openapi.yaml"],
  [generatedOperations, "identityOperationPaths"],
  [generatedGoModels, "Code generated by services/identity/tools/generate-types.mjs"],
  [generatedGoModels, "type ActorRoleView struct"],
  [generatedGoOperations, "IdentityOperationProvisionActorRole"],
]) {
  if (!body.includes(required)) failures.push("Identity generated client lineage missing " + required);
}
for (const forbidden of [
...
if (!challenge.includes("identity_operator_enrollment_tokens")) {
  failures.push("Identity challenge service missing identity_operator_enrollment_tokens");
}
if (challenge.includes("identity_managed_activation_codes") || challenge.includes("IssueManagedActivationCode")) {
  failures.push("Identity challenge service retains retired managed activation code references");
}
...
if (!readiness.includes("identity_operator_enrollment_tokens")) {
  failures.push("Identity readiness proof missing identity_operator_enrollment_tokens");
}
if (readiness.includes("identity_managed_activation_codes")) {
  failures.push("Identity readiness proof retains retired identity_managed_activation_codes");
}
