...
if (pickedUp.status !== 200 || inCustodyOrder.status !== 200 || inCustodyOrder.body?.order?.state !== "IN_CUSTODY" || deliveryProof.status !== 200 || deliveryProof.body?.state !== "PENDING" || !/^[0-9]{6}$/.test(String(deliveryProof.body?.code || ""))) fail("delivery proof was not disclosed exactly when the order entered captain custody", JSON.stringify({ pickupStatus: pickedUp.status, orderState: inCustodyOrder.body?.order?.state, proofStatus: deliveryProof.status, proofState: deliveryProof.body?.state, codeDisclosed: Boolean(deliveryProof.body?.code) }));
