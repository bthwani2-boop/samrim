...
const proposalBody = { id: proposalID, verticalId: verticalID, categoryId: childCategoryID, proposedName: `Runtime Proposal ${suffix}`, proposedBrand: "Samrim", proposedVariantTitle: "الافتراضي", proposedMeasurementKind: "DISCRETE", proposedBaseUnit: "COUNT", variantAttributeValues: [{ attributeId: enumAttributeID, valueKind: "ENUM", enumValue: "Dark" }], proposedIdentifierType: "SKU", proposedIdentifierValue: `PROPOSAL-${suffix}`, proposedImageUri: "https://example.com/proposal.jpg" };
...
const hiddenCartOffer = await request(dshBase, "PATCH", `/dsh/stores/${first.storeID}/offers/${offerAID}`, { token: first.accessToken, headers: partnerHeaders(`cart-offer-hide-${suffix}`, 2), body: discreteOffer(2100, "hidden") });
...
const restoredCartOffer = await request(dshBase, "PATCH", `/dsh/stores/${first.storeID}/offers/${offerAID}`, { token: first.accessToken, headers: partnerHeaders(`cart-offer-restore-${suffix}`, 3), body: discreteOffer(2100, "published") });
...
if (promotionCheckout.status !== 201 || promotionCheckout.body?.order?.subtotalAmountMinor !== 2100 || promotionCheckout.body.order.discountMinor !== 300 || promotionCheckout.body.order.promotionCode !== promotionCode || promotionCheckout.body.order.totalAmountMinor !== 1800) fail("promotion checkout did not snapshot the canonical discount", JSON.stringify(promotionCheckout));
...
const promotionAllocationCount = sql(`SELECT count(*) FROM wlt.customer_payment_allocations WHERE order_id='${sqlLiteral(promotionOrderID)}' AND subtotal_minor=2100 AND discount_minor=300 AND customer_payable_minor=1800 AND cash_amount_minor=1800`);
