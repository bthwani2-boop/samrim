...
await waitForSQL(`SELECT state FROM dsh.commerce_financial_handoff_outbox WHERE effect_type='DELIVERY_SETTLEMENT' AND order_id='${sqlLiteral(orderID)}' AND source_ref='${sqlLiteral(captainAssignmentID)}'`, "POSTED", "delivery settlement financial handoff did not reconcile");
expectSQL(`SELECT captain_actor_id || '|' || partner_actor_id || '|' || acting_actor_id FROM dsh.commerce_financial_handoff_outbox WHERE effect_type='DELIVERY_SETTLEMENT' AND order_id='${sqlLiteral(orderID)}'`, `${captainActorID}|${first.actorID}|${captainActorID}`, "delivery settlement financial handoff lost canonical actors");
