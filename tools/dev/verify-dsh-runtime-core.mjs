...
const multiCheckoutBody = { id: multiCheckoutID, children: [
  ...
  // City B has no delivery-fee policy. The matching address makes this a fee
  // failure after serviceability succeeds, rather than an unrelated cart error.
...
const failedChildOrderCount = sql(`SELECT count(*) FROM dsh.commerce_orders WHERE cart_id='${sqlLiteral(multiCartBID)}'`);
if (multiCheckout.status !== 201 || multiCheckout.body?.checkout?.state !== "PARTIAL_FAILURE" || multiCheckout.body.checkout.successfulChildCount !== 1 || multiCheckout.body.checkout.failedChildCount !== 1 || multiChildSuccess?.state !== "SUCCEEDED" || multiChildFailure?.state !== "FAILED" || multiChildFailure?.failureCode !== "DELIVERY_FEE_UNAVAILABLE" || multiCheckoutReplay.status !== 200 || multiCheckoutReplay.body?.idempotentReplay !== true || multiCheckoutReplay.body.checkout.id !== multiCheckoutID || multiCheckoutRead.status !== 200 || multiCheckoutRead.body?.checkout?.state !== "PARTIAL_FAILURE" || multiChildReadbackCount !== "2" || multiChildAllocationCount !== "1" || failedChildOrderCount !== "0") fail("multi-store fee-unavailable child did not converge to a definitive failure with canonical parent/order/WLT readback", JSON.stringify({ multiCheckout, multiCheckoutReplay, multiCheckoutRead, multiChildReadbackCount, multiChildAllocationCount, failedChildOrderCount }));
...
const partnerLegalNamePath = `/dsh/operator/actors/${encodeURIComponent(first.actorID)}/legal-name`;
const partnerLegalNameInput = { givenName: "محمد", secondName: "علي", thirdName: "صالح", familyName: "الحديدة", evidenceReference: `partner-identity-proof-${suffix}` };
const partnerLegalNameSubmitted = await request(dshBase, "POST", partnerLegalNamePath, { token: dshToken, headers: serviceHeaders(actingOperatorID, `partner-legal-name-${suffix}`), body: partnerLegalNameInput });
const partnerLegalNamePending = await request(dshBase, "GET", `${partnerLegalNamePath}/pending`, { token: dshToken, headers: { "X-Acting-Actor-ID": actingOperatorID } });
const partnerLegalNameSelfVerification = partnerLegalNameSubmitted.body?.legalName?.version
  ? await request(dshBase, "POST", `${partnerLegalNamePath}/${partnerLegalNameSubmitted.body.legalName.version}/verify`, { token: dshToken, headers: serviceHeaders(actingOperatorID, `partner-legal-name-self-verify-${suffix}`), body: { evidenceReference: `partner-identity-verification-${suffix}` } })
  : null;
const partnerLegalNameVerified = partnerLegalNameSubmitted.body?.legalName?.version
  ? await request(dshBase, "POST", `${partnerLegalNamePath}/${partnerLegalNameSubmitted.body.legalName.version}/verify`, { token: dshToken, headers: serviceHeaders(checkerOperatorID, `partner-legal-name-verify-${suffix}`), body: { evidenceReference: `partner-identity-verification-${suffix}` } })
  : null;
...
async function verifyPayoutBeneficiaryName(actorID, role) {
  ...
  const version = submitted.body?.legalName?.version;
  if (submitted.status !== 201 || submitted.body?.legalName?.actorId !== actorID || submitted.body.legalName.status !== "PENDING_VERIFICATION" || !version) fail(`${role} beneficiary legal name submission failed`, JSON.stringify(submitted));
  const verified = await request(dshBase, "POST", `${path}/${version}/verify`, { token: dshToken, headers: serviceHeaders(checkerOperatorID, `${role}-legal-name-verify-${suffix}`), body: { evidenceReference: `${role}-identity-verification-${suffix}` } });
