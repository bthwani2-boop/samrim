...
const missingModifierCheckout = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(`checkout-missing-modifier-${suffix}`, 2), body: { cartId: cartID, storeId: first.storeID, addressId: addressAID, fulfillmentMode: "BTHWANI_CAPTAIN" } });
...
const checkoutQuote = await request(dshBase, "POST", "/dsh/cart/quote", { token: client.accessToken, headers: { "X-Expected-Version": "3" }, body: { cartId: cartID, storeId: first.storeID, addressId: addressAID, fulfillmentMode: "BTHWANI_CAPTAIN" } });
if (checkoutQuote.status !== 200 || checkoutQuote.body?.quote?.cartId !== cartID || checkoutQuote.body.quote.storeId !== first.storeID || checkoutQuote.body.quote.addressId !== addressAID || checkoutQuote.body.quote.fulfillmentMode !== "BTHWANI_CAPTAIN" || checkoutQuote.body.quote.cartVersion !== 3 || checkoutQuote.body.quote.subtotalMinor !== mainOrderSubtotal || checkoutQuote.body.quote.deliveryFeeMinor !== deliveryFeeMinor || checkoutQuote.body.quote.totalAmountMinor !== mainOrderTotal || checkoutQuote.body.quote.currency !== "YER" || typeof checkoutQuote.body.quote.deliveryPolicyVersion !== "string" || !checkoutQuote.body.quote.deliveryPolicyVersion || typeof checkoutQuote.body.quote.quotedAt !== "string") fail("checkout quote did not expose the canonical subtotal, WLT delivery fee, and expected total", JSON.stringify(checkoutQuote));
const cartUnserviceable = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(`checkout-unserviceable-${suffix}`, 3), body: { cartId: cartID, storeId: first.storeID, addressId: addressBID, fulfillmentMode: "BTHWANI_CAPTAIN" } });
...
const unavailableCheckout = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(`checkout-offer-unavailable-${suffix}`, 3), body: { cartId: cartID, storeId: first.storeID, addressId: addressAID, fulfillmentMode: "BTHWANI_CAPTAIN" } });
...
const checkout = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(checkoutKey, 3), body: { cartId: cartID, storeId: first.storeID, addressId: addressAID, fulfillmentMode: "BTHWANI_CAPTAIN" } });
...
const checkoutReplay = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(checkoutKey, 3), body: { cartId: cartID, storeId: first.storeID, addressId: addressAID, fulfillmentMode: "BTHWANI_CAPTAIN" } });
...
const finalizedCaptainReservationCount = sql(`SELECT count(*) FROM wlt.captain_cod_reservations WHERE order_id='${sqlLiteral(orderID)}' AND payment_intent_id='${sqlLiteral(paymentIntentID)}' AND captain_actor_id='${sqlLiteral(activeCaptainActorID)}' AND amount_minor=${mainOrderTotal} AND state='FINALIZED' AND ledger_transaction_id IS NOT NULL`);
