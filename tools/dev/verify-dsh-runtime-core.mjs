...
expectSQL("SELECT count(*) FROM dsh.schema_migrations", "32", "DSH migration history is not v32");
...
console.log("DSH_SCHEMA_V32=PASS");
...
async function createApprovedPartner(phone, name, serviceCityId, origin = "control_panel") {
  ...
  if (approved.status !== 200 || approved.body?.case?.state !== "approved" || approved.body?.case?.store?.primaryVerticalId !== verticalID || approved.body?.case?.store?.deliveryOrigin?.latitude !== firstStoreOrigin.firstStoreLatitude || approved.body?.case?.store?.deliveryOrigin?.longitude !== firstStoreOrigin.firstStoreLongitude) fail("joining case approval did not atomically transfer the fixed store origin", JSON.stringify(approved));
...
const fieldApproved = await request(dshBase, "POST", `/dsh/joining-cases/${encodeURIComponent(fieldCaseID)}/review`, { token: dshToken, headers: serviceHeaders(actingOperatorID, `field-approve-${suffix}`, crypto.randomUUID(), 2), body: { decision: "approved" } });
if (fieldApproved.status !== 200 || fieldApproved.body?.case?.state !== "approved" || !fieldApproved.body?.case?.store?.id || fieldApproved.body?.case?.firstStoreLatitude !== firstStoreOrigin.firstStoreLatitude || fieldApproved.body?.case?.firstStoreLongitude !== firstStoreOrigin.firstStoreLongitude || fieldApproved.body?.case?.store?.deliveryOrigin?.latitude !== firstStoreOrigin.firstStoreLatitude || fieldApproved.body?.case?.store?.deliveryOrigin?.longitude !== firstStoreOrigin.firstStoreLongitude) fail("operator approval of Field-originated joining case did not preserve the fixed store origin", JSON.stringify(fieldApproved));
...
if (rejectedCheckout.status !== 201 || rejectedCheckout.body?.order?.paymentState !== "REQUIRES_COLLECTION" || rejectedCheckout.body.order.totalAmountMinor !== 2100 || typeof rejectedCheckout.body.order.paymentIntentId !== "string") fail("payment rejection checkout fixture failed", JSON.stringify(rejectedCheckout));
