...
function cleanup() {
  ...
  for (const orderID of orderIDs) {
    ...
    sql(`DELETE FROM wlt.ledger_entries WHERE transaction_id IN (SELECT ledger_transaction_id FROM wlt.captain_cod_reservations WHERE order_id='${value}' AND ledger_transaction_id IS NOT NULL)`);
    sql(`DELETE FROM wlt.ledger_transactions WHERE source_type='CAPTAIN_COD_RESERVATION' AND source_id IN (SELECT id FROM wlt.captain_cod_reservations WHERE order_id='${value}')`);
...
expectSQL("SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname='captain_operation_idempotency_operation_chk'", "CHECK ((operation = ANY (ARRAY['availability'::text, 'dispatch'::text, 'respond_offer'::text, 'reassign'::text, 'store_confirm'::text, 'pickup'::text, 'complete'::text, 'recover'::text])))", "Captain recovery operation is not canonical");
...
const deliveryFeePolicyCreate = await request(dshBase, "POST", "/dsh/operator/delivery-fee-policy", { token: dshToken, headers: serviceHeaders(actingOperatorID, `delivery-fee-policy-${suffix}`), body: { serviceCityId: cityA, baseFeeMinor: 100, distanceUnitMeters: 1000000, distanceRateMinor: 0, orderSizeUnitBaseUnits: 100, orderSizeRateMinor: 0, zoneSurchargeMinor: 0, roundingUnitMinor: 50 } });
...
const fieldAccessToken = await activateField(fieldPhone, `Fiel${suffix.slice(0, 4)}`);
...
const crossStoreMediaUpload = await request(dshBase, "POST", `/dsh/stores/${first.storeID}/products/${storeProductID}/media/upload`, { token: second.accessToken, headers: partnerHeaders(`cross-store-media-${suffix}`, 2), rawBody: partnerMediaUploadForm() });
const partnerMediaUpload = await request(dshBase, "POST", `/dsh/stores/${first.storeID}/products/${storeProductID}/media/upload`, { token: first.accessToken, headers: partnerHeaders(partnerMediaKey, 2), rawBody: partnerMediaUploadForm() });
...
const partnerMediaReplay = await request(dshBase, "POST", `/dsh/stores/${first.storeID}/products/${storeProductID}/media/upload`, { token: first.accessToken, headers: partnerHeaders(partnerMediaKey, 2), rawBody: partnerMediaUploadForm() });
const partnerMediaStale = await request(dshBase, "POST", `/dsh/stores/${first.storeID}/products/${storeProductID}/media/upload`, { token: first.accessToken, headers: partnerHeaders(`store-product-media-stale-${suffix}`, 2), rawBody: partnerMediaUploadForm() });
const partnerGalleryMediaKey = `store-product-gallery-${suffix}`;
const partnerGalleryMediaUpload = await request(dshBase, "POST", `/dsh/stores/${first.storeID}/products/${storeProductID}/media/upload`, { token: first.accessToken, headers: partnerHeaders(partnerGalleryMediaKey, 3), rawBody: (() => { const form = new FormData(); form.set("role", "gallery"); form.set("file", new Blob([partnerImageBytes], { type: "image/png" }), "local-world-rice-gallery.png"); return form; })() });
const partnerGalleryURI = partnerGalleryMediaUpload.body?.product?.media?.find((item) => item.role === "gallery")?.uri;
const partnerReplaceMedia = await request(dshBase, "PUT", `/dsh/stores/${first.storeID}/products/${storeProductID}/media`, { token: first.accessToken, headers: partnerHeaders(`store-product-gallery-remove-${suffix}`, 4), body: { media: [{ uri: partnerUploadedURI, role: "primary", ordinal: 0 }] } });
const partnerReplaceReplay = await request(dshBase, "PUT", `/dsh/stores/${first.storeID}/products/${storeProductID}/media`, { token: first.accessToken, headers: partnerHeaders(`store-product-gallery-remove-${suffix}`, 4), body: { media: [{ uri: partnerUploadedURI, role: "primary", ordinal: 0 }] } });
const crossStoreMediaReplace = await request(dshBase, "PUT", `/dsh/stores/${first.storeID}/products/${storeProductID}/media`, { token: second.accessToken, headers: partnerHeaders(`cross-store-media-replace-${suffix}`, 5), body: { media: [{ uri: partnerUploadedURI, role: "primary", ordinal: 0 }] } });
const partnerReplaceStale = await request(dshBase, "PUT", `/dsh/stores/${first.storeID}/products/${storeProductID}/media`, { token: first.accessToken, headers: partnerHeaders(`store-product-media-replace-stale-${suffix}`, 4), body: { media: [{ uri: partnerUploadedURI, role: "primary", ordinal: 0 }] } });
...
const variableProductInput = { canonicalName: `Variable Coffee ${suffix}`, verticalId: verticalID, scope: "SHARED", variantTitle: "وزن متغير", measurementKind: "VARIABLE_MEASURE", baseUnit: "GRAM", categoryIds: [childCategoryID], identifierType: "SKU", identifierValue: `VARIABLE-${suffix}`, imageUri: "https://example.com/variable-coffee.jpg" };
...
const linkedAllocationDBCount = sql(`SELECT count(*) FROM wlt.payment_allocations WHERE order_id='${sqlLiteral(orderID)}' AND payment_intent_id='${sqlLiteral(paymentIntentID)}' AND currency='YER' AND subtotal_minor=${mainOrderSubtotal} AND delivery_fee_minor=${deliveryFeeMinor} AND discount_minor=0 AND platform_subsidy_minor=0 AND internal_wallet_amount_minor=0 AND external_official_wallet_amount_minor=0 AND cash_amount_minor=${mainOrderTotal} AND cod_product_amount_minor=${mainOrderSubtotal} AND cod_delivery_amount_minor=${deliveryFeeMinor} AND total_minor=${mainOrderTotal} AND policy_version LIKE 'cod-current-v1;delivery=%'`);
...
if (linkedPaymentRead.status !== 200 || linkedPaymentRead.body?.paymentIntent?.state !== "REQUIRES_COLLECTION" || linkedPaymentRead.body.paymentIntent.amountMinor !== mainOrderTotal || linkedPaymentRead.body.paymentIntent.currency !== "YER" || linkedPaymentRead.body.paymentIntent.method !== "CASH_ON_DELIVERY" || linkedPaymentAuditCount !== "1" || linkedAllocation?.orderId !== orderID || linkedAllocation?.paymentIntentId !== paymentIntentID || linkedAllocation?.currency !== "YER" || linkedAllocation?.subtotalMinor !== mainOrderSubtotal || linkedAllocation?.deliveryFeeMinor !== deliveryFeeMinor || linkedAllocation?.discountMinor !== 0 || linkedAllocation?.platformSubsidyMinor !== 0 || linkedAllocation?.internalWalletAmountMinor !== 0 || linkedAllocation?.externalOfficialWalletAmountMinor !== 0 || linkedAllocation?.cashAmountMinor !== mainOrderTotal || linkedAllocation?.codProductAmountMinor !== mainOrderSubtotal || linkedAllocation?.codDeliveryAmountMinor !== deliveryFeeMinor || linkedAllocation?.totalMinor !== mainOrderTotal || !String(linkedAllocation?.policyVersion || "").startsWith("cod-current-v1;delivery=") || linkedAllocationDBCount !== "1" || linkedAllocationEventCount !== "1") fail("checkout did not link a canonical WLT payment allocation", JSON.stringify({ checkout, linkedPaymentRead, linkedPaymentAuditCount, linkedAllocationDBCount, linkedAllocationEventCount }));
...
const rejectedCartCreate = await request(dshBase, "POST", "/dsh/cart/lines", { token: client.accessToken, headers: partnerHeaders(`payment-reject-cart-${suffix}`, 0), body: { storeId: first.storeID, storeOfferId: offerAID, quantityBaseUnits: 1, selectedModifierOptionIds: [modifierOptionID] } });
...
const rejectedCheckout = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(`payment-reject-checkout-${suffix}`, 1), body: { cartId: rejectedCartID, storeId: first.storeID, addressId: addressAID } });
...
const clientCancelCartCreate = await request(dshBase, "POST", "/dsh/cart/lines", { token: client.accessToken, headers: partnerHeaders(`client-cancel-cart-${suffix}`, 0), body: { storeId: first.storeID, storeOfferId: offerAID, quantityBaseUnits: 1, selectedModifierOptionIds: [modifierOptionID] } });
...
const clientCancelCheckout = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(`client-cancel-checkout-${suffix}`, 1), body: { cartId: clientCancelCartID, storeId: first.storeID, addressId: addressAID } });
...
if (clientOrders.status !== 200 || !clientOrders.body?.orders?.some((item) => item.id === orderID) || clientOrderRead.status !== 200 || clientOrderRead.body?.order?.id !== orderID || partnerOrders.status !== 200 || !partnerOrders.body?.orders?.some((item) => item.id === orderID) || partnerOrderRead.status !== 200 || partnerOrderRead.body?.order?.id !== orderID || !clientOrderRead.body.order.lines?.[0]?.modifierSnapshots?.some((snapshot) => snapshot.optionId === modifierOptionID) || !partnerOrderRead.body.order.lines?.[0]?.attributeSnapshots?.some((snapshot) => snapshot.attributeId === enumAttributeID && snapshot.enumValue === "Dark")) fail("client/partner Order readback boundary failed", JSON.stringify({ clientOrders, clientOrderRead, partnerOrders, partnerOrderRead }));
...
if (firstDispatch.status !== 201 || firstDispatch.body?.offer?.state !== "offered" || firstDispatch.body.offer.captainActorId !== captainActorID) fail("Captain dispatch did not create an addressed offer", JSON.stringify(firstDispatch));
...
if (firstDispatchReplay.status !== 200 || firstDispatchReplay.body?.idempotentReplay !== true || firstDispatchReplay.body.offer.id !== firstCaptainOfferID || captainOffersAfterDispatch.status !== 200 || !captainOffersAfterDispatch.body?.offers?.some((offer) => offer.id === firstCaptainOfferID && offer.state === "offered")) fail("Captain dispatch idempotent readback failed", JSON.stringify({ firstDispatchReplay, captainOffersAfterDispatch }));
...
const confirmedHandoff = await request(dshBase, "POST", `/dsh/stores/${encodeURIComponent(first.storeID)}/orders/${encodeURIComponent(orderID)}/handoff`, { token: first.accessToken, headers: partnerHeaders(`captain-handoff-${suffix}`, 1), body: { assignmentId: captainAssignmentID } });
...
const confirmedHandoffReplay = await request(dshBase, "POST", `/dsh/stores/${encodeURIComponent(first.storeID)}/orders/${encodeURIComponent(orderID)}/handoff`, { token: first.accessToken, headers: partnerHeaders(`captain-handoff-${suffix}`, 1), body: { assignmentId: captainAssignmentID } });
...
const pickedUp = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/pickup`, { token: captainAccessToken, headers: partnerHeaders(`captain-pickup-${suffix}`, 2) });
...
const captainLocation = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/location`, { token: captainAccessToken, headers: locationHeaders(captainLocationKey), body: { latitude: 15.3701, longitude: 44.1911 } });
const captainLocationReplay = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/location`, { token: captainAccessToken, headers: locationHeaders(captainLocationKey), body: { latitude: 15.3701, longitude: 44.1911 } });
const wrongCaptainLocation = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/location`, { token: secondCaptainAccessToken, headers: locationHeaders(`captain-location-wrong-${suffix}`), body: { latitude: 15.3702, longitude: 44.1912 } });
const clientTracking = await request(dshBase, "GET", `/dsh/orders/${encodeURIComponent(orderID)}/tracking`, { token: client.accessToken });
const failedDelivery = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/complete`, { token: captainAccessToken, headers: partnerHeaders(`captain-failed-${suffix}`, 2), body: { result: "delivery_failed" } });
const failedOrder = await request(dshBase, "GET", `/dsh/orders/${encodeURIComponent(orderID)}`, { token: client.accessToken });
const captainAfterFailure = await request(dshBase, "GET", "/dsh/captains/me", { token: captainAccessToken });
const captainWalletAfterFailure = await request(wltBase, "GET", `/wlt/v1/captains/${encodeURIComponent(captainActorID)}/wallet-state`, { token: wltToken });
...
const recovered = await request(dshBase, "POST", `/dsh/captains/assignments/${encodeURIComponent(captainAssignmentID)}/recover`, { token: dshToken, headers: serviceHeaders(actingOperatorID, recoveryKey, crypto.randomUUID(), 3) });
...
const wrongProof = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/complete`, { token: captainAccessToken, headers: partnerHeaders(`captain-complete-proof-wrong-${suffix}`, 4), body: { result: "delivered", collectedAmountMinor: mainOrderTotal, deliveryProofCode: "000000" } });
...
const wrongCollection = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/complete`, { token: captainAccessToken, headers: partnerHeaders(`captain-complete-wrong-${suffix}`, 4), body: { result: "delivered", collectedAmountMinor: mainOrderTotal - 1, deliveryProofCode } });
const completed = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/complete`, { token: captainAccessToken, headers: partnerHeaders(`captain-complete-${suffix}`, 4), body: { result: "delivered", collectedAmountMinor: mainOrderTotal, deliveryProofCode } });
const completedReplay = await request(dshBase, "POST", `/dsh/captains/me/assignments/${encodeURIComponent(captainAssignmentID)}/complete`, { token: captainAccessToken, headers: partnerHeaders(`captain-complete-${suffix}`, 4), body: { result: "delivered", collectedAmountMinor: mainOrderTotal, deliveryProofCode } });
...
const partnerEarningRowCount = sql(`SELECT count(*) FROM wlt.partner_order_earnings WHERE order_id='${sqlLiteral(orderID)}' AND payment_intent_id='${sqlLiteral(paymentIntentID)}' AND partner_actor_id='${sqlLiteral(first.actorID)}' AND captain_actor_id='${sqlLiteral(captainActorID)}' AND gross_product_minor=${mainOrderSubtotal} AND delivery_fee_minor=${deliveryFeeMinor} AND commission_minor=650 AND partner_net_minor=3550`);
...
const captainWalletAfterDelivery = await request(wltBase, "GET", `/wlt/v1/captains/${encodeURIComponent(captainActorID)}/wallet-state`, { token: wltToken });
const finalizedCaptainReservationCount = sql(`SELECT count(*) FROM wlt.captain_cod_reservations WHERE order_id='${sqlLiteral(orderID)}' AND payment_intent_id='${sqlLiteral(paymentIntentID)}' AND captain_actor_id='${sqlLiteral(captainActorID)}' AND amount_minor=${mainOrderTotal} AND state='FINALIZED' AND ledger_transaction_id IS NOT NULL`);
...
const createdRemittance = await request(dshBase, "POST", `/dsh/captains/me/cash-liability/${encodeURIComponent(paymentIntentID)}/remit`, { token: captainAccessToken, headers: partnerHeaders(remittanceKey, collectedPaymentRead.body?.paymentIntent?.version), body: { amountMinor: mainOrderTotal, remittanceReference } });
const remittanceReplay = await request(dshBase, "POST", `/dsh/captains/me/cash-liability/${encodeURIComponent(paymentIntentID)}/remit`, { token: captainAccessToken, headers: partnerHeaders(remittanceKey, collectedPaymentRead.body?.paymentIntent?.version), body: { amountMinor: mainOrderTotal, remittanceReference } });
const remittanceKeyConflict = await request(dshBase, "POST", `/dsh/captains/me/cash-liability/${encodeURIComponent(paymentIntentID)}/remit`, { token: captainAccessToken, headers: partnerHeaders(remittanceKey, collectedPaymentRead.body?.paymentIntent?.version), body: { amountMinor: mainOrderTotal, remittanceReference: "different-reference" } });
...
const expiryCheckout = await request(dshBase, "POST", "/dsh/cart/checkout", { token: client.accessToken, headers: partnerHeaders(`captain-expiry-checkout-${suffix}`, 1), body: { cartId: expiryCartID, storeId: first.storeID, addressId: addressAID } });
...
const expiredOfferRepeat = await request(dshBase, "GET", "/dsh/captains/me/offers", { token: captainAccessToken });
...
const captainRoleBeforeDisable = await request(identityBase, "GET", `/internal/actors/${encodeURIComponent(captainActorID)}/roles/captain`, { token: identityDshToken });
...
const captainRoleAfterDisable = await request(identityBase, "GET", `/internal/actors/${encodeURIComponent(captainActorID)}/roles/captain`, { token: identityDshToken });
