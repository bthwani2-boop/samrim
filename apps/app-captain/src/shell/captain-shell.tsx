...
type CaptainSearchNavigation = {
  navigate: (screen: "offers", params: { focus: "search" }) => void;
...
export function createCaptainTabOptions(theme: ReturnType<typeof resolveTheme>) {
  ...
  const icons = { home: "home", offers: "offers", deliveries: "deliveries", wallet: "wallet", account: "account" } as const;
  return ({ route }: { route: { name: string } }) => ({
    ...
    header: ({ route: headerRoute, navigation }: { route: { params?: unknown }; navigation: unknown }) => {
      ...
      return <SafeAreaView edges={["top"]} style={styles.headerSafeArea}><CaptainHeader focus={params?.focus} searchQuery={params?.q} navigation={navigation as CaptainSearchNavigation} styles={styles} /></SafeAreaView>;
    },
...
function CaptainHeader({ focus: rawFocus, searchQuery: rawQuery, navigation, styles }: { focus?: string | string[] | undefined; searchQuery?: string | string[] | undefined; navigation: CaptainSearchNavigation; styles: ReturnType<typeof createStyles> }) {
  ...
  return <View style={styles.header}>{isSearchOpen ? <View style={styles.headerSearchActions}><BthwaniSearchField accessibilityLabel="البحث في العروض" autoCapitalize="none" autoCorrect={false} containerStyle={styles.headerSearchField} inputRef={searchInputRef} onChangeText={(value) => navigation.setParams({ q: value })} onClear={() => navigation.setParams({ q: "" })} placeholder="ابحث برقم الطلب أو المتجر أو العنوان" returnKeyType="search" value={searchQuery} /><BthwaniIconButton icon="close" label="إغلاق البحث" onPress={() => navigation.setParams({ focus: "", q: "" })} size={sizing.controlMd} tone="soft" /></View> : <><View style={styles.headerIdentity}><Text style={styles.brand}>بثواني · الكابتن</Text><Text style={styles.context}>الجاهزية والتوصيل</Text></View><View style={styles.headerActions}><BthwaniIconButton icon="search" label="البحث في العروض" onPress={() => navigation.navigate("offers", { focus: "search" })} size={sizing.controlMd} tone="soft" /><BthwaniIconButton icon="notifications" label="الإشعارات" onPress={() => router.push("/notifications" as Href)} size={sizing.controlMd} tone="soft" /><BthwaniIconButton icon="account" label="الحساب" onPress={() => router.push("/account" as Href)} size={sizing.controlMd} tone="soft" /></View></>}</View>;
}
...
function createStyles(theme: ReturnType<typeof resolveTheme>) {
  return StyleSheet.create({
    ...
    navigationItem: { alignItems: "center", borderRadius: radius.md, gap: spacing[1], justifyContent: "center", minHeight: sizing.controlLg, paddingHorizontal: spacing[1], transform: [{ translateY: -spacing[4] }] },
