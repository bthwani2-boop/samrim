...
export default function CaptainAccount() {
  ...
  return <View style={styles.container} accessibilityLabel="حساب الكابتن"><Text style={styles.eyebrow}>إدارة الحساب</Text><Text style={styles.title}>حساب الكابتن</Text><Text style={styles.description}>تظل الأهلية والتوفر والتوصيلات مرتبطة بحالة حسابك الحالية.</Text><BthwaniSurface tone="base" style={styles.card}><Text style={styles.cardTitle}>جلسة التشغيل</Text><Text style={styles.description}>الجلسة الحالية مفعّلة لهذا الجهاز.</Text></BthwaniSurface><StoreCaptainMemberships /><AppearancePicker /><BthwaniButton accessibilityLabel="تسجيل الخروج" busy={busy} label="تسجيل الخروج" onPress={() => void logout()} />{notice ? <Text accessibilityRole="alert" style={styles.notice}>{notice}</Text> : null}</View>;
}
