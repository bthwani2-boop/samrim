...
const theme = useAppearanceTheme();
  ...
  async function complete(assignment: CaptainAssignment, result: "delivered" | "delivery_failed", collectedAmountMinor?: number) {
    ...
    try {
      const token = await getUsableIdentityAccessToken();
      await captainClient().completeCaptainAssignment(token, assignment.id, { result, ...(collectedAmountMinor === undefined ? {} : { collectedAmountMinor }) }, assignment.version);
      await load();
    } catch (cause) {
