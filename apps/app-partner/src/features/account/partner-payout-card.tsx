...
export function PartnerPayoutCard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (actor.kind === "authenticated") {
        const raw = await SecureStore.getItemAsync(`bthwani.partner.payout.pending.v1.${encodeURIComponent(actor.identity.subject)}`);
  ...
  const request = async (mode: "FULL_AVAILABLE" | "SPECIFIED") => {
    ...
    try {
      ...
      const storageKey = `bthwani.partner.payout.pending.v1.${encodeURIComponent(actor.identity.subject)}`;
      const attempt = pendingAttempt ?? { mode, ...(parsed === undefined ? {} : { amountMinor: parsed }), idempotencyKey: `partner_payout_${Crypto.randomUUID()}`, correlationID: `partner_payout_corr_${Crypto.randomUUID()}` };
      await SecureStore.setItemAsync(storageKey, JSON.stringify(attempt));
      setPendingAttempt(attempt);
      await client().createOwnPayoutIntent(token, mode, parsed, attempt.idempotencyKey, attempt.correlationID);
