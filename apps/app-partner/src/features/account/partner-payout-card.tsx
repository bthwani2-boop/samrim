...
import { createDshMobileClient, type PartnerPayoutState } from "@bthwani/dsh";
...
export function PartnerPayoutCard() {
  ...
  const [state, setState] = useState<PartnerPayoutState | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState((await client().readOwnPartnerPayoutState(token)).state);
    } catch (cause) {
  ...
  const request = async (mode: "FULL_AVAILABLE" | "SPECIFIED") => {
    ...
    try {
      ...
      await client().createOwnPartnerPayoutIntent(token, mode, parsed);
