...
const theme = useAppearanceTheme();
  ...
  const [storeProductAttributeRules, setStoreProductAttributeRules] = useState<ReadonlyArray<CatalogAttributeRule>>([]);
  const [proposalAttributeRules, setProposalAttributeRules] = useState<ReadonlyArray<CatalogAttributeRule>>([]);
  const [storeProductRuleState, setStoreProductRuleState] = useState<"idle" | "loading" | "ready" | "error">("idle");
  const [proposalRuleState, setProposalRuleState] = useState<"idle" | "loading" | "ready" | "error">("idle");
  const [storeProductAttributeDrafts, setStoreProductAttributeDrafts] = useState<AttributeDrafts>({});
  const [proposalAttributeDrafts, setProposalAttributeDrafts] = useState<AttributeDrafts>({});
  const [storeProductEnumOptions, setStoreProductEnumOptions] = useState<Readonly<Record<string, ReadonlyArray<string>>>>({});
  ...
  useEffect(() => {
    let current = true;
    if (!storeProductCategoryID) { setStoreProductAttributeRules([]); setStoreProductRuleState("idle"); return () => { current = false; }; }
    setStoreProductAttributeRules([]); setStoreProductRuleState("loading");
    void dshClient().listPublicCatalogCategoryAttributeRules(storeProductCategoryID).then(async (rules) => { const pairs = await Promise.all(rules.filter((rule) => rule.valueKind === "ENUM").map(async (rule) => [rule.attributeId, (await dshClient().listPublicCatalogAttributeEnumOptions(rule.attributeId)).map((option) => option.optionValue)] as const)); if (current) { setStoreProductAttributeRules(rules); setStoreProductEnumOptions(Object.fromEntries(pairs)); setStoreProductRuleState("ready"); } }).catch((nextError) => { if (current) { setStoreProductRuleState("error"); reportError(nextError); setError(errorText(nextError)); } });
  ...
  async function createStoreProduct() {
    ...
    if (storeProductRuleState !== "ready") { setError("تعذر التحقق من خصائص التصنيف. أعد تحميل القواعد قبل إنشاء المنتج."); return; }
    const attributes = buildAttributeInputs(storeProductAttributeRules, storeProductAttributeDrafts);
    if (!attributes) { setError("أكمل الخصائص المطلوبة وتحقق من أنواع القيم قبل إنشاء المنتج."); return; }
    setBusy(true); setError("");
    try {
      const token = await getUsableIdentityAccessToken();
      const image = storeProductImage;
      const created = await dshClient().createStoreScopedProduct(token, storeId, { canonicalName: storeProductName, verticalId: storeProductVerticalID, scope: "STORE_SCOPED", storeId, variantTitle: "الافتراضي", measurementKind: storeProductMeasurementKind, baseUnit: storeProductBaseUnit, categoryIds: [storeProductCategoryID], attributeValues: attributes.productValues, variantAttributeValues: attributes.variantValues, ...(!image && storeProductImageURI.trim() ? { imageUri: storeProductImageURI.trim() } : {}) });
      if (image) {
        try {
