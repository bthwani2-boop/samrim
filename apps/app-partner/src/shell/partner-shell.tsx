...
export function createPartnerTabOptions(theme: ReturnType<typeof resolveTheme>) {
  ...
  const icons = { store: "store", orders: "orders", account: "account" } as const;
...
function PartnerHeader({ styles }: { styles: ReturnType<typeof createStyles> }) {
  return <View style={styles.header}><View><Text style={styles.brand}>بثواني · الشريك</Text><Text style={styles.context}>تشغيل المتجر والطلبات</Text></View><View style={styles.headerMark} accessibilityElementsHidden><View style={styles.headerMarkNavy} /><View style={styles.headerMarkOrange} /></View></View>;
}
