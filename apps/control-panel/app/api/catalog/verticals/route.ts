...
export async function GET() {
  ...
  try {
    return NextResponse.json(await listCatalogVerticals({ operatorActorId: identity.subject }), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  if (!body || Object.keys(body).some((key) => !["id", "nameAr", "nameEn", "active"].includes(key)) || (Object.keys(body).length !== 3 && Object.keys(body).length !== 4) || (body.id !== undefined && (typeof body.id !== "string" || !body.id.trim())) || typeof body.nameAr !== "string" || typeof body.nameEn !== "string" || typeof body.active !== "boolean" || !body.nameAr.trim() || !body.nameEn.trim()) {
    return errorResponse("INVALID_INPUT", "nameAr, nameEn and active are required; DSH generates id automatically", 400);
  }
  const input: CreateCommerceVerticalRequest = { ...(typeof body.id === "string" ? { id: body.id.trim() } : {}), nameAr: body.nameAr.trim(), nameEn: body.nameEn.trim(), active: body.active };
  try {
