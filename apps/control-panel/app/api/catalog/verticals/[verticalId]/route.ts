...
export async function PATCH(request: Request, context: { params: Promise<{ verticalId: string }> }) {
  ...
  const allowed = ["nameAr", "nameEn", "active", "expectedVersion", "reason"];
  if (!body || Object.keys(body).length !== allowed.length || Object.keys(body).some((key) => !allowed.includes(key)) || typeof body.nameAr !== "string" || typeof body.nameEn !== "string" || typeof body.active !== "boolean" || !Number.isInteger(body.expectedVersion) || Number(body.expectedVersion) < 1 || typeof body.reason !== "string" || body.reason.trim().length < 5 || body.reason.trim().length > 500) return errorResponse("INVALID_INPUT", "vertical update facts are invalid", 400);
  try {
    const { verticalId } = await context.params;
    const input: UpdateCommerceVerticalRequest = { nameAr: body.nameAr.trim(), nameEn: body.nameEn.trim(), active: body.active, expectedVersion: Number(body.expectedVersion), reason: body.reason.trim() };
