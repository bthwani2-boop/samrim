...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as { phone?: unknown; role?: unknown; action?: unknown; reason?: unknown; actorId?: unknown } | null;
  ...
  const explicitActorId = typeof body?.actorId === "string" ? body.actorId.trim() : undefined;
  ...
  try {
    ...
    const mutationOptions = { operatorActorId: identity.subject };
    if (action === "disable-role" || action === "enable-role") {
      const enabled = action === "enable-role";
      if (roleNames.has(role)) await setManagedRoleEnabled(phone, role as "partner" | "captain" | "field", enabled, reason, correlationId, mutationOptions);
      else await setIdentityRoleEnabled(phone, role, enabled, reason, mutationOptions);
    } else {
      let targetActorId = explicitActorId;
      if (!targetActorId && roleNames.has(role)) {
