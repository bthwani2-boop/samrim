...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as { actorId?: unknown; role?: unknown } | null;
  const actorId = typeof body?.actorId === "string" ? body.actorId.trim() : "";
  const role = typeof body?.role === "string" ? body.role.trim().toLowerCase() : "";
  if (!actorId || !managedRoles.has(role as ManagedActivationRole)) return NextResponse.json({ error: { code: "INVALID_INPUT", message: "actorId and managed role are required" } }, { status: 400, headers: { "Cache-Control": "no-store" } });
  try {
    await authorizeManagedReenrollment(actorId, role as "partner" | "captain" | "field", randomUUID());
