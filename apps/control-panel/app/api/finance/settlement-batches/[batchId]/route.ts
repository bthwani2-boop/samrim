...
import { dshErrorPayload, dshHttpStatus, isDshClientError, readOperatorSettlementBatch } from "../../../../../src/server/dsh/dsh-bff";
...
export async function GET(_request: Request, context: { params: Promise<{ batchId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await readOperatorSettlementBatch(batchId, { operatorActorId: identity.subject }), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    if (!isDshClientError(error)) return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "تعذر قراءة دفعة التسوية" } }, { status: 500 });
