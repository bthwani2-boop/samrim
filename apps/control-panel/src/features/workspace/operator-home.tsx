...
import { joiningCaseStateLabel, orderStateLabel, type JoiningCaseSummary, type OperatorOperation } from "@bthwani/dsh";
...
export function OperatorHome() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const operationsBody = await operationsResponse.json() as { operations?: ReadonlyArray<OperatorOperation> };
      ...
      {!loading && !error ? (
        <div className="home-work-areas">
          <section className="access-card home-work-area" aria-labelledby="home-operations-title">
            ...
            {operations.length === 0 ? <p className="empty-inline">لا توجد أعمال تشغيلية تحتاج عرضًا في القراءة الحالية.</p> : <ul className="home-list">{operations.slice(0, 4).map((item) => <li key={item.order.id}><Link href="/operations"><span><strong>{item.storeName}</strong><small>طلب يحتاج متابعة</small></span><strong>{orderStateLabel(item.order.state)}</strong></Link></li>)}</ul>}
