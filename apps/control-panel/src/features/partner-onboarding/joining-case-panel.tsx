...
export function JoiningCasePanel() {
  ...
  async function createCase() {
    const input = { contactPhoneE164: phone.trim(), businessName: businessName.trim(), firstStoreName: storeName.trim() };
    if (!input.contactPhoneE164 || input.businessName.length < 2 || input.firstStoreName.length < 2) {
    ...
    <section className="access-card" aria-labelledby="joining-case-title">
      ...
      {!current ? (
        <div className="access-form">
          <label className="field-label" htmlFor="joining-phone">رقم هاتف الشريك<input id="joining-phone" autoComplete="tel" disabled={busy} inputMode="tel" value={phone} onChange={(event) => { setPhone(event.target.value); clearResult(); }} placeholder="مثال: 967 77 000 100" /></label>
