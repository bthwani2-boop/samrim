...
export default function StoreDiscovery() {
  ...
  return <View style={styles.container}><Text style={styles.eyebrow}>اكتشاف العميل</Text><Text style={styles.title}>المتاجر المنشورة</Text><Text style={styles.muted}>هذه القائمة تأتي من DSH ولا تعرض إلا المتاجر ذات حالة النشر المنشورة.</Text><FlatList data={state.stores} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} renderItem={({ item }) => <Pressable accessibilityRole="button" accessibilityLabel={`فتح متجر ${item.name}`} onPress={() => void openStore(item)} style={styles.card}><Text style={styles.cardTitle}>{item.name}</Text><Text style={styles.cardMeta}>متجر منشور · إصدار {item.version}</Text></Pressable>} /></View>;
}
