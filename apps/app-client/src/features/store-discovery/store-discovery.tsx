...
export default function StoreDiscovery() {
  ...
  if (state.kind === "ready" && !selectedVerticalID) {
    const verticals = [...new Map(state.stores.filter((store) => store.primaryVerticalId).map((store) => [store.primaryVerticalId as string, store])).entries()];
    return <View style={styles.container}><Text style={styles.eyebrow}>اكتشاف العميل</Text><Text style={styles.title}>اختر المجال التجاري</Text><Text style={styles.muted}>تسلسل الاكتشاف: المدينة ← المجال التجاري ← المتجر.</Text><View style={styles.list}>{verticals.map(([verticalID, store]) => <Pressable key={verticalID} accessibilityRole="button" accessibilityLabel={`فتح مجال ${verticalID}`} onPress={() => setSelectedVerticalID(verticalID)} style={styles.card}><Text style={styles.cardTitle}>{verticalID}</Text><Text style={styles.cardMeta}>{state.stores.filter((item) => item.primaryVerticalId === verticalID).length} متجر منشور</Text></Pressable>)}</View></View>;
  }
  if (state.kind === "ready" && selectedVerticalID && !selected) {
    const stores = state.stores.filter((store) => store.primaryVerticalId === selectedVerticalID);
    return <View style={styles.container}><Pressable accessibilityRole="button" onPress={() => setSelectedVerticalID(null)}><Text style={styles.back}>‹ المجالات التجارية</Text></Pressable><Text style={styles.eyebrow}>المجال: {selectedVerticalID}</Text><Text style={styles.title}>اختر متجرًا</Text><View style={styles.list}>{stores.map((item) => <Pressable key={item.id} accessibilityRole="button" accessibilityLabel={`فتح متجر ${item.name}`} onPress={() => void openStore(item)} style={styles.card}><Text style={styles.cardTitle}>{item.name}</Text><Text style={styles.cardMeta}>{item.serviceCity.displayNameAr} · متجر منشور · إصدار {item.version}</Text></Pressable>)}</View></View>;
  }
