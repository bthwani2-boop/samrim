...
STATIC-CLOSURE-FIRST is the default execution order. Complete bounded discovery, implementation, root-cause repair, cutover, cleanup and every claim-appropriate source/static/type/unit/contract/schema check for the current authorized task before starting runtime, browser, device or agent-device proof. Do not enter expensive runtime proof after intermediate edits or small change units merely for reassurance. Defer real runtime/browser/device proof to the final coherent proof phase after static closure of the task, then repair only findings that require runtime truth and rerun only the evidence they invalidate. Enter runtime earlier only when the material fact is inherently runtime-only, runtime is required to diagnose a current blocker/root cause, or further static work would create material rework risk.
