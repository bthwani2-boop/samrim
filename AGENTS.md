...
**ROOT-FIRST LOCAL DEVELOPMENT LAW:** In `LOCAL DEVELOPMENT`, never preserve faulty code, data, schema, migration, design, state or path merely because it already exists or to minimize change. Diagnose the highest causal root first, choose the simplest and strongest complete canonical treatment, and execute it decisively—even when that means deletion, rewrite, restructuring, refoundation, remodeling, re-migration or recreating repository-owned local data. Do not waste time chasing symptom chains, repeatedly repairing consequences of a defective foundation, or layering patches/compatibility around a root that can be removed. Prefer one bounded root-cause investigation followed immediately by direct canonical repair, complete cutover, loser deletion, residue cleanup and claim-specific proof. Preserve only work/data/history with a proven current preservation, authority or safety obligation; the non-negotiable target is proven 100% closure of the authorized material scope, not preservation of existing mistakes.
